...
if st.sidebar.button('Plan my trip'):
    if place_input and duration_input and people_input:
        question = f"Plan a trip to {place_input} for {duration_input} days for {people_input} people. Include an itinerary, expected expenses, and any relevant images."
        response = handle_chat(question)
        if response:
            try:
                # Extract image URLs assuming they are prefixed with "Image URL:"
                image_urls = [line.split("Image URL:")[1].strip() for line in response.split('\n') if "Image URL:" in line]
                data = [item.split(',') for item in response.replace("Image URL:", "").split(';') if ',' in item]
                df = pd.DataFrame(data, columns=['Day', 'Activity', 'Cost'])
                st.subheader("Here's your trip plan in a table format:")
                st.table(df)
                # Display images
                for url in image_urls:
                    st.image(url, caption="Trip Image")
            except Exception as e:
                st.error(f"An error occurred while parsing the data or displaying images: {str(e)}")
        else:
